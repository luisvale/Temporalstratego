...
class AccountInvoiceImport(models.TransientModel):
    ...
    def parse_xml_invoice(self, xml_root):
        if xml_root.tag:
            try:
                ...
                return self.parse_fe_cr_invoice(xml_root, doc_type)
    ...
    @api.model
    def parse_fe_cr_invoice(self, xml_root, doc_type):
        """Parse UBL Invoice XML file"""
        namespaces = xml_root.nsmap
        inv_xmlns = namespaces.pop(None)
        namespaces["inv"] = inv_xmlns
        logger.debug("XML file namespaces={}").format(namespaces)
        ...
        inv_type = "in_invoice"
        if doc_type == "NotaCreditoElectronica":
            inv_type = "in_refund"
        ...
        origin = False
        supplier_dict = self.fe_cr_parse_party(
            xml_root.xpath("inv:Emisor", namespaces=namespaces)[0], namespaces
        )
        company_dict_full = self.fe_cr_parse_party(
            xml_root.xpath("inv:Receptor", namespaces=namespaces)[0], namespaces
        )
        company_dict = {}
        ...
        for iline in inv_line_xpath:
            line_vals = self.parse_fe_cr_invoice_line(iline, counters, namespaces)
        ...
        if float_compare(total_line, counters["lines"], precision_digits=prec):
            logger.warning(
                "The gloabl LineExtensionAmount {} doesn't match the "
                "sum of the amounts of each line ({}). It can "
                "have a diff of a few cents due to sum of rounded values vs "
                "rounded sum policies."
            ).format(total_line, counters["lines"])
        ...
        logger.info("Result of CR FE XML parsing: {}".format(res))
        return res

    @api.model
    def _prepare_create_invoice_vals(self, parsed_inv, import_config=False):
        (vals, import_config) = super(AccountInvoiceImport, self)._prepare_create_invoice_vals(
        ...
        if len(all_companies) > 1:  # multi-company setup
            ...
            if not company_id:
                logger.error(
                    ...
                    "Ignoring this email.".format(msg_dict["email_to"], msg_dict["cc"])
                )
                return
        ...
        if msg_dict.get("attachments"):
            ...
            for attachment in msg_dict["attachments"]:
                if attachment.fname.endswith(".xml"):
                    ...
                    namespaces["inv"] = inv_xmlns
                    doc_type = re.search(
                        "FacturaElectronica|TiqueteElectronico|NotaCreditoElectronica|NotaDebitoElectronica|MensajeHacienda",
                        xml_root.tag,
                    ).group(0)
                    if doc_type != "MensajeHacienda":
            ...
            for clave in invoices:
                ...
                if "invoice_attachment" in invoices[clave]:
                    ...
                    logger.info("Attachment {}: {}. Trying to import it as an invoice").format(
                        i, invoice_filename
                    )
                    parsed_inv = self.parse_invoice(invoice_file_b64, invoice_filename)
                    if (
                        filetype
                        and filetype[0] in ["application/xml", "text/xml"]
                        and parsed_inv != {}
                    ):
                        ...
                        if existing_inv:
                            logger.warning(
                                "Mail import: this supplier invoice already exists in Odoo (ID {} number {} supplier number {})"
                            ).format(
                                existing_inv.id,
                                existing_inv.number,
                        ...
                        if not import_configs:
                            logger.warning(
                                "Mail import: missing Invoice Import Configuration for partner '{}'."
                            ...
                            logger.info(
                                "There are {} invoice import configs for partner {}. Using the first one '{}''".format(
                                    len(import_configs),
