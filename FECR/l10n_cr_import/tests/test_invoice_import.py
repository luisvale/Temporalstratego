    ...
    def test_import_out_invoice(self):
        ...
        for import_config in self.all_import_config:
            if not import_config["invoice_line_method"].startswith("nline"):
                continue
            inv = self.env["account.invoice.import"].create_invoice(parsed_inv, import_config)
            prec = inv.currency_id.rounding
            self.assertFalse(float_compare(inv.amount_untaxed, 30.66, precision_rounding=prec))
            self.assertFalse(float_compare(inv.amount_total, 30.97, precision_rounding=prec))
