...
class AccountInvoiceImport(models.TransientModel):
    ...
    invoice_id = fields.Many2one("account.move", string="Draft Supplier Invoice to Update")
    ...
    def default_get(self, fields_list):
        ...
        if self._context.get("default_partner_id") and not self._context.get(
    ...
    def parse_pdf_invoice(self, file_data):
        ...
        for xml_filename, xml_root in list(xml_files_dict.items()):
            logger.info("Trying to parse XML file {}".format(xml_filename))
    ...
    def _prepare_create_invoice_vals(self, parsed_inv, import_config=False):
        ...
        start_end_dates_installed = hasattr(ailo, "start_date") and hasattr(ailo, "end_date")
        if parsed_inv["move_type"] in ("out_invoice", "out_refund"):
        ...
        currency = bdio._match_currency(parsed_inv.get("currency"), parsed_inv["chatter_msg"])
        journal_id = (
            aio.with_context(type=parsed_inv["type"], company_id=company_id)._default_journal().id
        )
        vals = {
            "partner_id": partner.id,
            "currency_id": currency.id,
            "move_type": parsed_inv["move_type"],
            "company_id": company_id,
            "invoice_origin": parsed_inv.get("invoice_origin"),
        ...
        if not config:
            if not partner.invoice_import_ids:
                raise UserError(_("Missing Invoice Import Configuration on partner '{}'.")).format(
        ...
        if config["invoice_line_method"].startswith("1line"):
            ...
            for line in parsed_inv["lines"]:
                ...
                if not il_vals.get("account_id") and il_vals.get("product_id"):
                    ...
                    raise UserError(
                        _("Account missing on product '{}' or on it's related " "category '{}'.")
                    ).format(product.display_name, product.categ_id.display_name)
                if line.get("name"):
                    il_vals["name"] = line["name"]
                elif not il_vals.get("name"):
                    il_vals["name"] = _("MISSING DESCRIPTION")
                if start_end_dates_installed:
                    il_vals["start_date"] = line.get("date_start") or parsed_inv.get("date_start")
                ...
                il_vals.update(
                    {
                        "quantity": line["qty"],
                        "price_unit": line["price_unit"],  # FIXME for tax incl
                    }
                )
                vals["invoice_line_ids"].append((0, 0, il_vals))
        # Write analytic account + fix syntax for taxes
        aacount_id = config.get("account_analytic") and config["account_analytic"].id or False
        if aacount_id:
    ...
    def pre_process_parsed_inv(self, parsed_inv):
        ...
        if "chatter_msg" not in parsed_inv:
            parsed_inv["chatter_msg"] = []
        if parsed_inv.get("move_type") in ("out_invoice", "out_refund"):
            return parsed_inv
        prec_ac = self.env["decimal.precision"].precision_get("Account")
        prec_pp = self.env["decimal.precision"].precision_get("Product Price")
        prec_uom = self.env["decimal.precision"].precision_get("Product Unit of Measure")
        if "amount_tax" in parsed_inv and "amount_untaxed" not in parsed_inv:
            parsed_inv["amount_untaxed"] = parsed_inv["amount_total"] - parsed_inv["amount_tax"]
        ...
        # method b)
        if not parsed_inv.get("move_type"):
            parsed_inv["move_type"] = "in_invoice"  # default value
        if (
            parsed_inv["move_type"] == "in_invoice"
            and float_compare(parsed_inv["amount_total"], 0, precision_digits=prec_ac) < 0
        ):
            parsed_inv["move_type"] = "in_refund"
                    ...
                    if len(line["taxes"]) > 1:
                        raise UserError(
                            _(
                                ...
                                "several VAT taxes on the same line ({}). We do "
                                "not support this scenario for the moment."
                            )
                        ).format(line.get("name"))
        ...
        for line in parsed_inv.get("lines", []):
            ...
            line["price_unit"] = float_round(line["price_unit"], precision_digits=prec_pp)
        logger.debug("Result of invoice parsing parsed_inv{}").format(parsed_inv)
    ...
    def import_invoice(self):
        ...
        wiz_vals = {
            "partner_id": partner.id,
            "invoice_type": parsed_inv["move_type"],
            "currency_id": currency.id,
        ...
        if existing_inv:
            raise UserError(
                _(
                    ...
                    "Supplier Invoice Number is '{}' and it's Odoo number "
                    "is '{}'"
                )
            ).format(parsed_inv.get("invoice_number"), existing_inv.number)
        ...
        if self.import_config_id:  # button called from 'config' step
            ...
            if not import_configs:
                ...
                logger.info(
                    ("There are {} invoice import configs for partner {}").format(
        ...
        if not wiz_vals.get("import_config_id"):
            ...
            action = iaao.for_xml_id("account_invoice_import", "account_invoice_import_action")
            ...
            logger.debug("draft_same_supplier_invs={}").format(draft_same_supplier_invs)
    ...
    def create_invoice_action(self, parsed_inv=None, import_config=None):
        ...
        invoice.message_post(_("This invoice has been created automatically via file import"))
    ...
    def create_invoice(self, parsed_inv, import_config=False):
        ...
        logger.debug("Invoice vals for creation: {}").format(vals)
    ...
    def _prepare_global_adjustment_line(self, diff_amount, invoice, import_config):
        ...
        logger.debug("Prepared global ajustment invoice line {}").format(il_vals)
        return il_vals
    ...
    def post_process_invoice(self, parsed_inv, invoice, import_config):
        ...
        if (
            ...
            for i in range(len(parsed_inv["lines"])):
                ...
                if float_compare(odoo_subtotal, parsed_subtotal, precision_rounding=prec):
                    diff_amount = float_round(
                        parsed_subtotal - odoo_subtotal, precision_rounding=prec
                    )
                    logger.info(
                        "Price subtotal difference found on invoice line {} "
                        "(source:{}, odoo:{}, diff:{}).".format(
                            i + 1, parsed_subtotal, odoo_subtotal, diff_amount
                        )
                    )
                    copy_dict = {
                        "name": _("Adjustment on {}").format(iline.name),
                        "quantity": 1,
                        "price_unit": diff_amount,
                    }
        ...
        if float_compare(
            ...
            logger.info(
                ("Amount untaxed difference found " "(source: {}, odoo:{}, diff:{})").format(
                    parsed_inv["amount_untaxed"], invoice.amount_untaxed, diff_amount
                )
            )
            il_vals = self._prepare_global_adjustment_line(diff_amount, invoice, import_config)
        ...
        if float_compare(invoice.amount_total, parsed_inv["amount_total"], precision_rounding=prec):
            ...
            if not invoice.tax_line_ids:
                ...
                invoice.message_post(_("The total tax amount has been forced to {} {} ")).format(
                ...
                invoice.message_post(
                    _(
                        "The total tax amount has been forced to {} {} "
                        "(amount computed by Odoo was: {} {})."
                    )
                ).format(tax_amount, cur_symbol, initial_tax_amount, cur_symbol)
    ...
    def update_invoice_lines(self, parsed_inv, invoice, seller):
        ...
        for eline, cdict in list(compare_res["to_update"].items()):
            ...
            if cdict.get("price_unit"):
                ...
                # FIX cdict["price_unit"][1]
                write_vals["price_unit"] = cdict["price_unit"][1]
            if write_vals:
                eline.write(write_vals)
        if compare_res["to_remove"]:
            to_remove_label = [
                "{} {} x {}".format(line.quantity, line.uom_id.name, line.product_id.name)
                for line in compare_res["to_remove"]
            ]
            chatter.append(_("{} invoice line(s) deleted: {}")).format(
        ...
        if compare_res["to_add"]:
            ...
            for add in compare_res["to_add"]:
                ...
                to_create_label.append(
                    "{} {} x {}".format(new_line.quantity, new_line.uom_id.name, new_line.name)
                )
            chatter.append(_("{} new invoice line(s) created: {}")).format(
    ...
    def _prepare_create_invoice_line(self, product, uom, import_line, invoice):
        ...
        vals = {
            f: new_line._fields[f].convert_to_write(new_line[f], new_line) for f in new_line._cache
        }
        vals.update(
            {
    ...
    def update_invoice(self):
        ...
        parsed_inv = self.parse_invoice(self.invoice_file, self.invoice_filename)
        logger.error("MAB - 01 - parsed_inv: {}").format(parsed_inv)

        if self.partner_id:
            # True if state='update' ; False when state='update-from-invoice'
            parsed_inv["partner"]["recordset"] = self.partner_id
            logger.error("MAB - 02 - parsed_inv:{}").format(parsed_inv["partner"])
        partner = bdio._match_partner(
            parsed_inv["partner"], parsed_inv["chatter_msg"], partner_type="supplier"
        )
        logger.error("MAB - 03 - parsed_inv: {}").format(partner)
        partner = partner.commercial_partner_id
        logger.error("MAB - 04 - parsed_inv: {}").format(partner)
        if partner != invoice.commercial_partner_id:
            raise UserError(
                _(
                    "The supplier of the imported invoice ({}) is different from "
                    "the supplier of the invoice to update ({})."
                )
            ).format(partner.name, invoice.commercial_partner_id.name)
        ...
        if currency != invoice.currency_id:
            raise UserError(
                _(
                    "The currency of the imported invoice ({}) is different from "
                    "the currency of the existing invoice ({})"
                )
            ).format(currency.name, invoice.currency_id.name)
        vals = self._prepare_update_invoice_vals(parsed_inv, partner)
        logger.debug("Updating supplier invoice with vals={}").format(vals)
        ...
        bdio.post_create_or_update(parsed_inv, invoice)
        logger.info(
            "Supplier invoice ID {} updated via import of file {}".format(
                invoice.id, self.invoice_filename
            )
        )
        invoice.message_post(
            _("This invoice has been updated automatically via the import " "of file {}")
        ).format(self.invoice_filename)
        action = iaao.for_xml_id("account", "action_invoice_tree2")
        action.update(
            {
                "view_mode": "form,tree,calendar,graph",
    ...
    def multi_xpath_helper(self, xml_root, xpath_list, namespaces, isdate=False, isfloat=False):
        ...
        for xpath in xpath_list:
            ...
            if xpath_res and xpath_res[0].text:
                if isdate:
                    if xpath_res[0].attrib and xpath_res[0].attrib.get("format") != "102":
    ...
    def message_new(self, msg_dict, custom_values=None):
        logger.info(
            ...
            "From: {}, Subject: {}, Date: {}, Message ID: {}. Executing "
            "with user {} ID {}".format(
        ...
        company_id = False
        all_companies = self.env["res.company"].search_read([], ["invoice_import_email"])
            ...
            for company in all_companies:
                if company["invoice_import_email"]:
                    ...
                    if company_dest_email in msg_dict.get(
                        ...
                        logger.info(
                            "Matched with {}: importing invoices in company "
                            "ID {}".format(company_dest_email, company_id)
                        )
                        break
            if not company_id:
                logger.error(
                    ...
                    "not found as destination of this email (to: {}, cc: {}). "
                    "Ignoring this email.".format(msg_dict.get("email_to"), msg_dict.get("cc"))
                )
                return
        ...
        if msg_dict.get("attachments"):
            ...
            for attach in msg_dict["attachments"]:
                logger.info(
                    "Attachment {}: {}. Trying to import it as an invoice".format(i, attach.fname)
                )
                parsed_inv = self.parse_invoice(base64.b64encode(attach.content), attach.fname)
                partner = bdio._match_partner(parsed_inv["partner"], parsed_inv["chatter_msg"])
                ...
                if existing_inv:
                    logger.warning(
                        ...
                        "in Odoo (ID {} number {} supplier number {})".format(
                            existing_inv.id, existing_inv.number, parsed_inv.get("invoice_number")
                        )
                    )
                    continue
                ...
                if not import_configs:
                    logger.warning(
                        ...
                        "for partner '{}' ID {}."
                    ).format(partner.display_name, partner.id)
                    continue
                elif len(import_configs) == 1:
                    import_config = import_configs.convert_to_import_config()
                else:
                    logger.info(
                        "There are {} invoice import configs for partner {}. "
                        "Using the first one '{}''".format(
                            len(import_configs), partner.display_name, import_configs[0].name
                        )
                    )
                ...
                invoice.message_post(
                    _(
                        ...
                        "<b>{}</b> on {} with subject <i>{}</i>."
                    ).format(
                        msg_dict.get("email_from"), msg_dict.get("date"), msg_dict.get("subject")
                    )
                )
        else:
